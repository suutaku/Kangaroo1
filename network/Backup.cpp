...
FILE *Kangaroo::ReadHeader(std::string fileName, uint32_t *version, int type) {
  ...
  if (head != type) {
    ...
    if (head == HEADKS) {
      fread(&nbLoadedWalk, sizeof(uint64_t), 1, f);
      ::printf("ReadHeader: %s is a compressed kangaroo only file [2^%.2f "
               "kangaroos]\n",
               fileName.c_str(), log2((double)nbLoadedWalk));
    } else if (head == HEADW) {
...
void Kangaroo::FetchWalks(uint64_t nbWalk, std::vector<int128_t> &kangs, Int *x,
                          Int *y, Int *d) {
...
void Kangaroo::FectchKangaroos(TH_PARAM *threads) {
  ...
  if (nbLoadedWalk > 0) {
    ...
    for (int i = 0; i < nbCPUThread; i++) {
      ...
      if (!saveKangarooByServer)
        FetchWalks(CPU_GRP_SIZE, threads[i].px, threads[i].py,
                   threads[i].distance);
      else
        FetchWalks(CPU_GRP_SIZE, kangs, threads[i].px, threads[i].py,
                   threads[i].distance);
    }
    ...
    for (int i = 0; i < nbGPUThread; i++) {
      ...
      if (!saveKangarooByServer)
        ...
        FetchWalks(n, kangs, threads[id].px, threads[id].py,
                   threads[id].distance);
    }
#endif
    ...
    ::printf("FectchKangaroos: [2^%.2f kangaroos loaded] [%.0f created] [%s]\n",
             log2((double)nbSaved), (double)created,
             GetTimeStr(eFetch - sFetch).c_str());
  }
...
// ----------------------------------------------------------------------------
bool Kangaroo::SaveHeader(string fileName, FILE *f, int type,
                          uint64_t totalCount, double totalTime) {
...
void Kangaroo::SaveServerDPStats() {
  ...
  ctimeBuff = ctime(&now);
  ::printf("done [%.1f MB] [%s] %s", (double)size / (1024.0 * 1024.0),
           GetTimeStr(t1 - t0).c_str(), ctimeBuff);

  saveCountRequest = false;
}

void Kangaroo::SaveWork(string fileName, FILE *f, int type, uint64_t totalCount,
                        double totalTime) {
...
void Kangaroo::SaveWork(uint64_t totalCount, double totalTime,
    ...
    if (saveKangarooByServer) {
      ...
      for (int i = 0; i < nbThread; i++) {
        ...
        for (uint64_t n = 0; n < threads[i].nbKangaroo; n++) {
          HashTable::Convert(&threads[i].px[n], &threads[i].distance[n], n % 2,
                             &h, &X, &D);
          kangs.push_back(D);
        }
      }
