...
bool Kangaroo::MergePartition(TH_PARAM *p) {
  ...
  for (uint32_t h = hStart; h < hStop && !endOfSearch; h++) {
    ...
    int mStatus = HashTable::MergeH(h, f1, f2, f, &hDP, &hDuplicate, &d1,
                                    &type1, &d2, &type2);
    switch (mStatus) {
    case ADD_OK:
      break;
...
bool Kangaroo::MergeWorkPartPart(std::string &part1Name,
  ...
  if (!partIsEmpty) {
    ...
    if (v1 != v2) {
      ::printf(
          "MergeWorkPartPart: cannot merge workfile of different version\n");
      ::fclose(f2);
      return true;
    }
    ...
    if (!k1.equals(k2)) {
      ...
      ::printf(
          "MergeWorkPartPart: key differs, multiple keys not yet supported\n");
      ::fclose(f2);
      return true;
    }
  ...
  THREAD_HANDLE *thHandles =
      (THREAD_HANDLE *)malloc(nbThread * sizeof(THREAD_HANDLE));
...
bool Kangaroo::FillEmptyPartFromFile(std::string &partName,
                                     std::string &fileName, bool printStat) {
...
bool Kangaroo::MergeWorkPart(std::string &partName, std::string &file2,
                             bool printStat) {
