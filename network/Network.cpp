...
string GetNetworkError() { return string(strerror(errno)); }
...
int Kangaroo::Read(SOCKET sock, char *buf, int bufsize,
...
bool Kangaroo::HandleRequest(TH_PARAM *p) {
  ...
  while (p->isRunning) {
    ...
    nbRead = Read(p->clientSock, (char *)(&cmdBuff), 1,
                  (int)(CLIENT_TIMEOUT * 1000.0));
    if (nbRead <= 0) {
      CLIENT_ABORT();
    }
    ...
    case SERVER_SETKNB: {
      GET("nbKangaroo", p->clientSock, &p->nbKangaroo, sizeof(uint64_t),
          ntimeout);
    ...
    case SERVER_LOADKANG: {
      ...
      GET("fileNameLenght", p->clientSock, &strSize, sizeof(uint32_t),
          ntimeout);
      if (strSize >= 256) {
      ...
      if (f == NULL) {
        ...
        ::printf("%s\n", ::strerror(errno));
        PUT("nbKangaroo", p->clientSock, &nbKangaroo, sizeof(uint64_t),
            ntimeout);
        break;
      }
    ...
    case SERVER_SAVEKANG: {
      ...
      GET("fileNameLenght", p->clientSock, &fileNameSize, sizeof(uint32_t),
          ntimeout);
      if (fileNameSize >= 256) {
        ::printf("\nFileName too long (MAX=256) %s\n", p->clientInfo);
        CLIENT_ABORT();
    ...
    case SERVER_SENDDP: {
      ...
      if (head.nbDP == 0) {
        ...
        ::printf("%d DP from %s, worker \"%s\" (total %lu), \n", head.nbDP,
                 p->clientInfo, key.c_str(), head.workerName);
...
// #define VALIDITY_POINT_CHECK
          ...
          for (uint32_t i = 0; i < head.nbDP; i++) {
            ...
            bool ok = (hC == h) && (P.x.bits64[0] == dp[i].x.i64[0]) &&
                      (P.x.bits64[1] == dp[i].x.i64[1]);
            if (!ok) {
              if (kType == TAME) {
                ::printf("\nWrong TAME point from: %s [dp=%d PID=%u thId=%u "
                         "gpuId=%u]\n",
                         p->clientInfo, i, head.processId, head.threadId,
                         head.gpuId);
              } else {
                ::printf("\nWrong WILD point from: %s [dp=%d PID=%u thId=%u "
                         "gpuId=%u]\n",
                         p->clientInfo, i, head.processId, head.threadId,
                         head.gpuId);
              }
...
void Kangaroo::AcceptConnections(SOCKET server_soc) {
  ...
  while (true) {
    ...
    if ((clientSock =
             accept(server_soc, (struct sockaddr *)&client_add, &len)) < 0) {
      ...
      char info[256];
      ::sprintf(info, "%s:%d", inet_ntoa(client_add.sin_addr),
...
void Kangaroo::RunServer() {
  ...
  if (initDPSize < 0) {
    ::printf("Error: Server must be launched with a specified number of "
             "distinguished bits (-d)\n");
    exit(-1);
  }
  SetDP(initDPSize);
  ...
  /* Reuse Address */
  int32_t yes = 1;
  if (setsockopt(serverSock, SOL_SOCKET, SO_REUSEADDR, (char *)&yes,
                 sizeof(yes)) < 0) {
...
bool Kangaroo::ConnectToServer(SOCKET *retSock) {
  ...
  if (connectStatus < 0) {
    ...
    if (getsockopt(sock, SOL_SOCKET, SO_ERROR, (char *)&socket_err, &serrlen) !=
        0) {
#else
  ...
  if (setsockopt(sock, p->p_proto, TCP_NODELAY, (char *)&flag, sizeof(flag)) ==
      -1) {
  ...
  while (!ok) {
    ...
    while (!isConnected) {
      ...
      if (isConnected) {
        ...
        nbWrite =
            Write(serverConn, (char *)&totalRW, sizeof(uint64_t), ntimeout);
        if (nbWrite <= 0) {
          if (nbWrite < 0)
            ::printf("\nSendToServer(SetKNb): %s\n", lastError.c_str());
          serverStatus = "Not OK";
...
bool Kangaroo::GetKangaroosFromServer(std::string &fileName,
                                      std::vector<int128_t> &kangs) {
  ...
  if (!endOfSearch) {
    ...
    PUT("fileNameLenght", serverConn, &fileNameSize, sizeof(uint32_t),
        ntimeout);
...
bool Kangaroo::SendKangaroosToServer(std::string &fileName,
                                     std::vector<int128_t> &kangs) {
...
void Kangaroo::AddConnectedClient() { connectedClient++; }

void Kangaroo::RemoveConnectedClient() { connectedClient--; }

void Kangaroo::RemoveConnectedKangaroo(uint64_t nb) { totalRW -= nb; }
