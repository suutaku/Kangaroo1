...
uint32_t Kangaroo::CheckHash(uint32_t h, uint32_t nbItem, HashTable *hT,
                             FILE *f) {
  ...
  for (uint32_t i = 0; i < nbItem; i++) {
    ...
    ok = (hC == h) && (S[i].x.bits64[0] == e->x.i64[0]) &&
         (S[i].x.bits64[1] == e->x.i64[1]);
    if (!ok)
      nbWrong++;
    // if(!ok) {
...
void Kangaroo::Check(std::vector<int> gpuId, std::vector<int> gridSize) {
  ...
  */
  ...
  if (useGpu) {
    ...
    Int k1;
    k1.SetBase16(
        "5B3F38AF935A3640D158E871CE6E9666DB862636383386EE0000000000123000");
        ...
        if (IsDP(cpuPx[i].bits64[3])) {
          ...
          if (found) {
            ...
            ::printf("[%d] %s [0x%016I64X]\n", j,
                     gpuFound[j].x.GetBase16().c_str(), gpuFound[j].kIdx);
#else
            ::printf("[%d] %s [0x%" PRIx64 "]\n", j,
                     gpuFound[j].x.GetBase16().c_str(), gpuFound[j].kIdx);
#endif
            ::printf("[%d] %s \n", i,
                     cpuPx[gpuFound[i].kIdx].GetBase16().c_str());
            return;
          }
        }
      }
