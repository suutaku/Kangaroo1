...
Kangaroo::Kangaroo(Secp256K1 *secp, int32_t initDPSize, bool useGpu,
                   ...
                   string outputFile, bool splitWorkfile, std::string countFile,
...
bool Kangaroo::IsDP(uint64_t x) { return (x & dMask) == 0; }
...
bool Kangaroo::Output(Int *pk, char sInfo, int sType) {
  ...
  std::string addr = secp->GetAddress(P2PKH, false, PR);
...
bool Kangaroo::CheckKey(Int d1, Int d2, uint8_t type) {
  ...
  std::string addr = secp->GetAddress(P2PKH, false, P);
...
void Kangaroo::ComputeExpected(double dp, double *op, double *ram,
  ...
  *ram =
      (double)sizeof(HASH_ENTRY) * (double)HASH_SIZE +    // Table
