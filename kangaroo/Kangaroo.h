...
public:
  Kangaroo(Secp256K1 *secp, int32_t initDPSize, bool useGpu,
           ...
           int wtimeout, int sport, int ntimeout, std::string serverIp,
           std::string outputFile, bool splitWorkfile, std::string countFile,
           int countPeriod, std::string &workerName);
  ...
  bool FillEmptyPartFromFile(std::string &partName, std::string &fileName,
                             bool printStat);
...
private:
  ...
  void SetDP(int size);
  void CreateHerd(int nbKangaroo, Int *px, Int *py, Int *d, int firstType,
                  bool lock = true);
  ...
  void SaveWork(std::string fileName, FILE *f, int type, uint64_t totalCount,
                double totalTime);
  void SaveWork(uint64_t totalCount, double totalTime, TH_PARAM *threads,
                int nbThread);
  void SaveServerWork();
  void SaveServerDPStats();
  void FetchWalks(uint64_t nbWalk, Int *x, Int *y, Int *d);
  void FetchWalks(uint64_t nbWalk, std::vector<int128_t> &kangs, Int *x, Int *y,
                  Int *d);
  void FectchKangaroos(TH_PARAM *threads);
  FILE *ReadHeader(std::string fileName, uint32_t *version, int type);
  bool SaveHeader(std::string fileName, FILE *f, int type, uint64_t totalCount,
                  double totalTime);
  ...
  bool SendKangaroosToServer(std::string &fileName,
                             std::vector<int128_t> &kangs);
  bool GetKangaroosFromServer(std::string &fileName,
                              std::vector<int128_t> &kangs);
