...
std::string Secp256K1::GetAddress(int type, bool compressed, Point &pubKey) {
  ...
  // SHA256(address, 21, address + 21);
  SHA256(address, 21, tmp);
  SHA256(tmp, 32, address + 21);

  // Base58
  return EncodeBase58(address, address + 25);
}

void Secp256K1::GetHash160(int type, bool compressed, Point &pubKey,
                           unsigned char *hash) {
  ...
  case BECH32: {
    ...
    RIPEMD160(shapk, SHA256_DIGEST_LENGTH, hash);
...
uint8_t Secp256K1::GetByte(std::string &str, int idx) {
  ...
  if (sscanf(tmp, "%X", &val) != 1) {
    printf("ParsePublicKeyHex: Error invalid public key specified (unexpected "
           "hexadecimal digit)\n");
    exit(-1);
  }
...
bool Secp256K1::ParsePublicKeyHex(std::string str, Point &ret,
  ...
  if (str.length() < 2) {
    printf("ParsePublicKeyHex: Error invalid public key specified (66 or 130 "
           "character length)\n");
    return false;
  }
  ...
  case 0x02:
    if (str.length() != 66) {
      printf("ParsePublicKeyHex: Error invalid public key specified (66 "
             "character length)\n");
      return false;
    }
    for (int i = 0; i < 32; i++)
  ...
  case 0x04:
    if (str.length() != 130) {
      printf("ParsePublicKeyHex: Error invalid public key specified (130 "
             "character length)\n");
      exit(-1);
    }
    for (int i = 0; i < 32; i++)
  ...
  if (!EC(ret)) {
    printf("ParsePublicKeyHex: Error invalid public key specified (Not lie on "
           "elliptic curve)\n");
    return false;
  }
