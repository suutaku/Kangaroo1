...
void printUsage() {
  ...
  printf(" -g g1x,g1y,g2x,g2y,...: Specify GPU(s) kernel gridsize, default is "
         "2*(MP),2*(Core/MP)\n");
  printf(" -d: Specify number of leading zeros for the DP method (default is "
         "auto)\n");
  printf(" -t nbThread: Secify number of thread\n");
  printf(" -dpcount countfile: Specify file to save running DP count stats "
         "into (server only)\n");
  printf(" -dpi countInverval: Periodic interval (in seconds) for saving DP "
         "count stats (server only)\n");
  printf(" -wname workerName: Specify worker name, up to 32 alphanumeric or "
         "underscore characters max, (client only)\n");
  printf(" -w workfile: Specify file to save work into (current processed key "
         "only)\n");
  printf(" -i workfile: Specify file to load work from (current processed key "
         "only)\n");
  ...
  printf(" -wpartcreate name: Create empty partitioned work file (name is a "
         "directory)\n");
  printf(" -wcheck worfile: Check workfile integrity\n");
  printf(" -m maxStep: number of operations before give up the search "
...
int main(int argc, char *argv[]) {
  ...
  if (workerName.length() > 32 ||
      workerName.find_first_not_of(
          "abcdefghijklmnopqrstuvwxyzABCDEFGHIJKLMNOPQRSTUVWXYZ01234567890_") !=
          std::string::npos) {
    printf("Error: invalid worker name, must have only alphanumreic and "
  ...
  Kangaroo *v = new Kangaroo(
      secp, dp, gpuEnable, workFile, iWorkFile, savePeriod, saveKangaroo,
      saveKangarooByServer, maxStep, wtimeout, port, ntimeout, serverIP,
      outputFile, splitWorkFile, countFile, countPeriod, workerName);
  if (checkFlag) {
