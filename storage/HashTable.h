...
// We store only 128 (+18) bit a the x value which give a probabilty a wrong
// collision after 2^73 entries

typedef struct {

  int128_t x; // Poisition of kangaroo (128bit LSB)
  int128_t
      d; // Travelled distance (b127=sign b126=kangaroo type, b125..b0 distance
...
public:
  HashTable();
  HashTable(std::string redis_server);
  ...
  static void Convert(Int *x, Int *d, uint32_t type, uint64_t *h, int128_t *X,
                      int128_t *D);
  static int MergeH(uint32_t h, FILE *f1, FILE *f2, FILE *fd, uint32_t *nbDP,
                    uint32_t *duplicate, Int *d1, uint32_t *k1, Int *d2,
                    uint32_t *k2);
